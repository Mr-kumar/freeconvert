...
import { validateImageFile, verifyMagicBytes } from "@/lib/validateFile";
...
export function ImageUploader({
  multiple = false,
  label = multiple ? "Add images" : "Add image",
}: ImageUploaderProps) {
  const {
    inputFile,
    batchFiles,
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDrop: async (files) => {
      async function validate(file: File) {
        ...
        if (!validation.valid) {
          setError(validation.error || "This image is not supported.");
          return false;
        }
    ...
    <div className="space-y-3">
      ...
      {activeFiles.length > 0 ? (
        <div className="overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--surface-2)]">
          {activeFiles.slice(0, 6).map((file, index) => (
            <div
              className="flex items-center gap-3 border-b border-[var(--border)] p-3 last:border-b-0"
              key={`${file.name}-${file.lastModified}-${index}`}
            >
              <FileImage className="h-4 w-4 shrink-0 text-[var(--accent)]" />
